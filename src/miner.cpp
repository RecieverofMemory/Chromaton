...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ChromatonMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ChromatonMiner : ProcessBlock, block not accepted");
    }
...
{
    LogPrintf("ChromatonMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("chromaton-miner");
    ...
    try { while (true) {
        ...
        LogPrintf("Running ChromatonMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("ChromatonMiner terminated\n");
        throw;
    }
}
